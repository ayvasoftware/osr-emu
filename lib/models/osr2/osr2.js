...
export default class OSR2Model {
  ...
  load () {
    const receiverMaterial = new MeshLambertMaterial({ color: 0x3f5173 });
    const baseMaterial = new MeshLambertMaterial({ color: 0x1E1E1E });
    ...
    return { meshes: this.meshes, orientation: this.#orientation };
  }
  ...
  preRender (axes, scale) {
    ...
    this.meshes.modCase.rotation.set(deg(90) + pitch, roll, axes['R0'] * deg(240 * R0_SCALE) - deg(120  * R0_SCALE));
  }
  ...
  postRender (axes) {
    ...
    this.#calculateArm(axes, this.meshes.rightArm, this.#armAxisLength, -67, this.#armYOffset, 18.22, -65, 0, 0);
    this.#calculateArm(axes, this.meshes.leftArm, this.#armAxisLength, 67, this.#armYOffset, 18.22, 63, 0, 0);
    this.#calculateArm(axes, this.meshes.supportArm, this.#supportAxisLength, 66.385, -33.243 + this.#armYOffset, 18.558, 64, -40, 0);
  }
