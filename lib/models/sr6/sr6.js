...
export default class SR6Model {
  ...
  #forwardKinematics (axes, scale) {
    ...
    const rightIntersectionPoint =  this.#computeIntersectionPoint(upperRightLink.position, lowerRightLink.position)
    const leftIntersectionPoint = this.#computeIntersectionPoint(upperLeftLink.position, lowerLeftLink.position);
  ...
  #computeIntersectionPoint (upperLinkPosition, lowerLinkPosition) {
    ...
    return intersection.center.add(tangentVector.clone().multiplyScalar(intersection.radius));
  }
